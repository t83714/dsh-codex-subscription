...
function createSubscriptionRpcHandler({ authHandler, usageReader, resetCreditService, preferences, diagnosticsReader, modelCatalog, originalImages, resolveInheritedOriginal, onAccountSelected }) {
	return async (endpoint, payload, signal) => {
...
function apply(ctx) {
	const settings = ctx.settings.register(SETTINGS_NAMESPACE, z.object({
		[AUTO_QUOTA_RETRY_FIELD]: z.boolean().default(true),
